...
function ContactPage() {
  ...
  const featured = destinations[3] ?? destinations[0];
  const moments = destinations.slice(0, 3).map((destination) => ({
    ...
    <>
      ...
      <main>
        <section className="relative overflow-hidden bg-[#0e1726] text-white">
          <img
            src={MEDIA.destinations?.[featured.slug] ?? featured.image}
            alt={featured.name}
          ...
          <div className="section-shell relative grid gap-10 py-16 md:py-20 lg:grid-cols-[0.96fr_1.04fr] lg:items-end">
            <div>
              <span className="eyebrow mb-5 text-[#d7aa73]">Start the conversation</span>
              <h1 className="mb-7 max-w-4xl text-balance text-5xl font-black leading-[1.02] md:text-7xl">
                Tell us the kind of moments you want to experience.
              </h1>
              <p className="max-w-2xl text-lg leading-9 text-white/76">
                Every unforgettable journey starts as a feeling. A planner will personally respond
                within 24 hours and shape the first route around what you want to remember.
              </p>
