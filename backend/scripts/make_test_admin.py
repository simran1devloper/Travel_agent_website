#!/usr/bin/env python3
"""Seed a test admin account and print its login token.

Run from the project root:
    python backend/scripts/make_test_admin.py

Or from backend/:
    python scripts/make_test_admin.py
"""
...
def main() -> None:
    ...
    if row:
        ...
        conn.commit()
        print(f"[updated] Existing account upgraded to admin (id={customer_id})")
    else:
        ...
        print(f"[created] Admin account seeded (id={customer_id})")
    ...
    print()
    print("  Access:")
    print("  - All admin endpoints  (/admin/*, /packages, /destinations, etc.)")
    print("  - Moderator endpoints  (admin role satisfies moderator check)")
    print()
    print("  Usage:")
    print('  curl -H "Authorization: Bearer <token>" http://localhost:8000/admin/stats')
    print("=" * 60)
